...
RAW_YELP_QUERIES = [
    """
    SELECT COUNT(u.data->>'user_id') AS cnt, (u.data->>'user_id') AS uid, (u.data->>'name') as name, CAST(u.data->>'average_stars' AS FLOAT) AS avg_stars
    FROM users u, users r
    WHERE (r.data->>'user_id') = (u.data->>'user_id')
      AND (u.data->>'name') IS NOT NULL
      AND CAST(u.data->>'average_stars' AS FLOAT) IS NOT NULL
      AND (r.data->>'review_id') IS NOT NULL
    GROUP BY (u.data->>'user_id'), (u.data->>'name'), CAST(u.data->>'average_stars' AS FLOAT)
    ORDER BY cnt DESC
    LIMIT 10;
    """,
    """
    SELECT COUNT(DISTINCT (r.data->>'user_id')) AS cnt, (r.data->>'business_id') AS bid
    FROM users r
    WHERE CAST(r.data->>'date' AS DATE) > DATE '2017-01-01'
      AND CAST(r.data->>'date' AS DATE) < DATE '2018-01-01'
      AND (r.data->>'review_id') IS NOT NULL
    GROUP BY (r.data->>'business_id')
    ORDER BY cnt DESC
    LIMIT 10;
    """,
    """
    WITH citycount (city, cnt, userid) AS (
        SELECT DISTINCT (b.data->>'city') AS city, COUNT(*) AS cnt, (r.data->>'user_id') AS userid
        FROM users b, users r
        WHERE (r.data->>'business_id') = (b.data->>'business_id')
          AND (b.data->>'city') IS NOT NULL
          AND (r.data->>'review_id') IS NOT NULL
        GROUP BY (b.data->>'city'), (r.data->>'user_id')
    )
    ...
    """,
    """
    SELECT (b.data->>'city') AS city, COUNT((r.data->>'review_id')) / COUNT(DISTINCT (r.data->>'business_id')) AS avg_reviews_per_business, COUNT((r.data->>'review_id')) AS reviews, COUNT(DISTINCT (r.data->>'business_id')) AS businesses
    FROM users b, users r
    WHERE (r.data->>'business_id') = (b.data->>'business_id')
      AND (b.data->>'city') IS NOT NULL
      AND (r.data->>'review_id') IS NOT NULL
