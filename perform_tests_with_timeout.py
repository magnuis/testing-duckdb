...
TIMEOUT_SECONDS = 100  # Timeout value in seconds
...
def execute_query_with_timeout(con, query, timeout):
    def run_query():
        con.execute(query)
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        return execution_time, timed_out
...
def perform_tests(
    ...
    '''Perform the tests with timeout'''
    ...
    for i, query in enumerate(queries, start=1):
        ...
        for j in range(5):  # Execute the query 5 times
            execution_time, timed_out = execute_query_with_timeout(
                con, query, TIMEOUT_SECONDS)
            if timed_out:
                print(f"Timeout occurred for Query {i} Iteration {j}")
        ...
        print(f"Query {i} Average Execution Time (last 4 runs): {avg_time:.4f} seconds")
    return results_df
